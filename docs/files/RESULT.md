...
# Defense — BOS Scrubber Results (Threadripper)
...
- `W=1`: scrubber active, passthrough (no reordering)
- `W>=2`: buffer up to W IDs before flushing in permuted order

**Threat model:**
- Seed is a **server-side secret**
- Window size `W` may be observable (bursty output)

## Defense files (expected)
...
| W | Samples | ODS AUC | Bootstrap AUC mean | 95% CI | Bootstrap BalAcc mean | 95% CI |
|---:|---:|---:|---:|---|---:|---|
| 0 (baseline) | 200 | 0.800 | 0.800 | [0.731, 0.863] | 0.791 | [0.734, 0.845] |
...
> **TODO:** Fill with measured numbers after running the scrubber sweep (see `REPRODUCE.md` Section 7).

---

## Overhead vs W

| W | Total runtime (50 runs) | Relative overhead |
|---:|---:|---:|
| 0 (baseline) | TODO | 1.00x |
| 1 | TODO | TODO |
| 2 | TODO | TODO |
| 4 | TODO | TODO |
| 8 | TODO | TODO |
...
> **TODO:** Fill with measured numbers. Measure wall-clock time for 50 runs with `time`.
...
As `W` increases:
- **AUC should decrease** (more entropy in output order ⇒ harder to distinguish CPU vs MEM)
- **Latency/overhead should increase** (IDs are held longer before release)

The "sweet spot" is the smallest `W` that brings AUC close to chance (≈0.5) with acceptable overhead.

---

## Interpretation (placeholder)

> **TODO:** After measurements, write interpretation here.
>
> Expected narrative:
> - W=1 should be similar to baseline (no reordering)
> - W=4 or W=8 may already collapse AUC significantly
> - W=16 or W=20 (equal to n) should bring AUC near chance
> - Overhead is expected to be minimal (scrubber is O(W) memory, O(W log W) per flush)
